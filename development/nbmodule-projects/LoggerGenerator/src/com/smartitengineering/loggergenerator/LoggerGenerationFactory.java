...
public class LoggerGenerationFactory {
		...
		private static void convertToLogFromSysOut(
				...
				hasStaticImport =
				hasStaticImport(stateContainer, imports, hasStaticImport, clazz,
								workingCopy, make);
				final boolean staticallyImported = hasStaticImport;
				Level level = (Level) stateContainer.getValue(LEVEL);
				...
				switch (expressionKind) {
						case METHOD_INVOCATION:
								...
								final ExpressionTree methodSelect =
																		 methodInvocationTree.getMethodSelect();
								if (methodSelect.getKind().equals(Kind.MEMBER_SELECT)) {
										MemberSelectTree memberSelectTree =
																		 (MemberSelectTree) methodSelect;
										final ExpressionTree memberExpression =
																				 memberSelectTree.getExpression();
										if (memberSelectTree.getIdentifier().contentEquals(PRINT) || memberSelectTree.getIdentifier().
														contentEquals(PRINTLN)) {
												sout =
												handlePrintMethods(memberSelectTree, memberExpression,
																unitTree,
																staticallyImported);
										}
								}
								if (sout) {
										replaceSysoutWithLog(make, level, methodInvocationTree,
														clazz, workingCopy);
								}
								break;
						default:
		...
		private static boolean handlePrintMethods(final MemberSelectTree memberSelectTree,
																							final ExpressionTree memberExpression,
																							final CompilationUnitTree unitTree,
																							final boolean staticallyImported) {
				boolean sout = false;
				...
				StringBuilder invocationMemberSelect =
											new StringBuilder(memberSelectTree.getIdentifier());
				switch (memberExpression.getKind()) {
						case IDENTIFIER:
								IdentifierTree idTree =
															 (IdentifierTree) memberExpression;
								final Trees instance = Trees.instance(task);
								final TreePath path = instance.getPath(unitTree, idTree);
								Element element = instance.getElement(path);
								sout =
								element.accept(new SimpleElementVisitor6<Boolean, Void>() {

										@Override
										public Boolean visitVariable(VariableElement e,
																								 Void p) {
												return e.asType().toString().
																equals(PrintStream.class.getName()) &&
															 e.getModifiers().
																contains(Modifier.STATIC) && staticallyImported;
										}
								}, null);
								invocationMemberSelect.insert(0, '.');
								invocationMemberSelect.insert(0, idTree.getName());
								break;
						case MEMBER_SELECT:
								MemberSelectTree selectTree =
																 (MemberSelectTree) memberExpression;
								makeMemberSelectString(invocationMemberSelect, selectTree);
								if (invocationMemberSelect.toString().startsWith(SYSTEM_OUT)) {
										sout = true;
								}
								break;
				}
		...
		private static boolean hasStaticImport(final StateContainer stateContainer,
																					 final List<? extends ImportTree> imports,
																					 boolean hasStaticImport,
																					 final ClassTree clazz,
																					 final WorkingCopy workingCopy,
																					 final TreeMaker make)
				...
				if (stateContainer != null) {
						if (stateContainer.isInitialized()) {
								hasStaticImport =
								((Boolean) stateContainer.getValue("hasStaticImport")).
												booleanValue();
						}
						else {
		...
		private static void makeMemberSelectString(StringBuilder builder,
																							 MemberSelectTree selectTree) {
				builder.insert(0, '.');
				builder.insert(0, selectTree.getIdentifier());
				if (selectTree.getExpression().getKind().equals(Kind.MEMBER_SELECT)) {
						makeMemberSelectString(builder, (MemberSelectTree) selectTree.
										getExpression());
				}
				else {
						if (selectTree.getExpression().getKind().equals(Kind.IDENTIFIER)) {
								builder.insert(0, '.');
								builder.insert(0, ((IdentifierTree) selectTree.getExpression()).
												getName());
						}
				}
		}
		...
		protected static Object[] addLoggerToClass(final boolean ignoreExisting,
				...
				if (!hasLogger) {
						...
						AssignmentTree assignmentTree =
										...
										getName()),
										"getLogger"),
										Collections.<ExpressionTree>singletonList(make.Literal(
										className))));
				...
				StringBuilder content =
											new StringBuilder(
								"{ java.util.logging.Handler[] handlers = ").append(loggerName).
								append(".getHandlers();").
								append(
								"boolean hasConsoleHandler = false;" +
				...
				MethodTree logMethod = make.Method(
								make.Modifiers(new HashSet(modifiers)),
								"log", returnType,
								Collections.<TypeParameterTree>emptyList(),
								params,
								Collections.<ExpressionTree>emptyList(),
								content.toString(), null);
				if (Collections.binarySearch(memberMethods, logMethod, methodComparator) <
						0) {
						modifiedClazz = make.insertClassMember(modifiedClazz == null ? clazz
																									 : modifiedClazz, position++,
										logMethod);
				}
				/**
		...
		public static void convertSysOutToLog(final WorkingCopy workingCopy,
																					...
																					final Level level)
						throws IOException {
				final SysOutNodeTraversalListenerImpl sysOutNodeTraversalListenerImpl =
																							new SysOutNodeTraversalListenerImpl();
				sysOutNodeTraversalListenerImpl.setValue(LEVEL, level);
				sysOutNodeTraversalListenerImpl.setValue(SET_LEVEL, setLevel);
				sysOutNodeTraversalListenerImpl.setValue(IGNORE_EXISTING, ignoreExisting);
				injectCodeWithListener(workingCopy, sysOutNodeTraversalListenerImpl);
		}
		...
		public static void addLogs(final WorkingCopy workingCopy,
				...
				shareableStateContainer.setValue(LEVEL, level);
				shareableStateContainer.setValue(SET_LEVEL, setLevel);
				shareableStateContainer.setValue(IGNORE_EXISTING, ignoreExisting);
				injectCodeWithListener(workingCopy,
		...
		public static void addLogToMethodBlock(ClassTree clazzTree,
				...
				if (returnType != null && returnType.getKind().
								equals(Kind.PRIMITIVE_TYPE) && ((PrimitiveTypeTree) returnType).
								getPrimitiveTypeKind().
								equals(TypeKind.VOID)) {
						List<ExpressionTree> exitLogArguments =
																 new ArrayList<ExpressionTree>();
						//Add the level
						exitLogArguments.add(make.MemberSelect(make.Identifier(Level.class.
										getName()),
										"FINEST"));
						//Add the sys out args
						exitLogArguments.add(make.Literal(new StringBuilder(
										"Exiting method: ").append(method.getName().
										toString()).
										toString()));
		...
		private static void addMethodForLoggingMethod(String loggerName,
				...
				MethodTree logMethod =
									 make.Method(make.Modifiers(new HashSet(modifiers)),
								"logMethod",
		...
		private static void addLogBeforeReturnStatement(ClassTree classTree,
				...
				if (encapsulatingReturnKind.equals(Kind.BLOCK)) {
						...
						addLogBeforeReturnWhichIsInABlock(true, oldBlockTree, returnTree,
										make,
						...
						switch (encapsulatingReturnKind) {
								...
								case CASE:
										BlockTree newBlockTree =
															make.Block(Collections.singletonList(returnTree),
														false);
										workingCopy.rewrite(returnTree, newBlockTree);
										addLogBeforeReturnWhichIsInABlock(false, newBlockTree,
														returnTree,
														make,
														classTree, workingCopy);
										break;
		...
		private static void addLogBeforeReturnWhichIsInABlock(boolean isBlock,
				...
				List<ExpressionTree> returnLogArguments =
														 new ArrayList<ExpressionTree>();
				//Add the level
				returnLogArguments.add(make.MemberSelect(make.Identifier(Level.class.
								getName()),
								"FINEST"));
				ExpressionTree returnExpression = returnTree.getExpression();
				returnExpression =
				returnExpression != null ? returnExpression : make.Literal(
								"Empty Expression!");
				//Add the sys out args
				returnLogArguments.add(make.Binary(Kind.PLUS, make.Literal(
								"Returning... "),
		...
		private static void addLogBeforeThrowStatement(ClassTree classTree,
				...
				if (encapsulatingThrowKind.equals(Kind.BLOCK)) {
						...
						switch (encapsulatingThrowKind) {
								...
								case CASE:
										BlockTree newBlockTree =
															make.Block(Collections.singletonList(throwTree),
														false);
										workingCopy.rewrite(throwTree, newBlockTree);
										addLogBeforeThrowWhichIsInABlock(false, newBlockTree,
														throwTree, make,
														classTree, workingCopy);
										break;
								default:
		...
		private static void addLogBeforeThrowWhichIsInABlock(boolean isBlock,
				...
				List<ExpressionTree> logThrowArgs =
														 new ArrayList<ExpressionTree>();
				//Add the level
				logThrowArgs.add(make.MemberSelect(
		...
		private static void replaceSysoutWithLog(final TreeMaker make,
																						 Level level,
																						 MethodInvocationTree methodInvocationTree,
																						 final ClassTree clazz,
																						 final WorkingCopy workingCopy) {
				...
				newLogMethodInvocationTree =
				make.MethodInvocation(Collections.<ExpressionTree>emptyList(),
								make.MemberSelect(make.Identifier(clazz.getSimpleName()), "log"),
								arguments);
				workingCopy.rewrite(methodInvocationTree, newLogMethodInvocationTree);
		}

		private static class SysOutNodeTraversalListenerImpl
						extends AbstractNodeTraversalListener {

				public SysOutNodeTraversalListenerImpl() {
						super();
				}

				public SysOutNodeTraversalListenerImpl(
								StateContainer preInitializedStateContainer) {
				...
				public void notifyAboutNode(NodeTraversalEvent event) {
						...
						convertToLogFromSysOut((ExpressionTree) event.getCurrentNode(),
										event.getWorkingCopy().getCompilationUnit(),
										event.getTreeMaker(),
		...
		private static class MethodNodeTraversalListenerImpl
				...
				public MethodNodeTraversalListenerImpl(
								StateContainer preInitializedStateContainer) {
						super(preInitializedStateContainer);
				}
		...
		private static class ReturnNodeTraversalListenerImpl
				...
				public ReturnNodeTraversalListenerImpl(
								StateContainer preInitializedStateContainer) {
						super(preInitializedStateContainer);
				}
		...
		private static class ThrowNodeTraversalListenerImpl
				...
				public ThrowNodeTraversalListenerImpl(
								StateContainer preInitializedStateContainer) {
						super(preInitializedStateContainer);
				}
